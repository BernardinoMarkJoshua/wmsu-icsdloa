<?php 
    ...
    } else if ($_SESSION['ACHIEVER_REMOVE_STATUS'] != 'yes') {
        echo '<script>window.location.href="achievers.php"</script>';
    } 

    $student_id = $_SESSION['ACHIEVER_STUDENT_ID_REMOVE'];
    $_SESSION['ACHIEVER_REMOVE_STATUS'] = 'no';
    $email = $_SESSION['ACHIEVER_EMAIL_REMOVE'];
    $subject= 'Removed from achievers';
    ...
    <script type="text/javascript">
        ...
        function sendEmail() {
            ...
            if (isNotEmpty(name) && isNotEmpty(email) && isNotEmpty(subject) && isNotEmpty(student_id) && isNotEmpty(body)) {
                $.ajax({
                   url: 'sendEmail_Admin_Achiever.php',
                   method: 'POST',
                   dataType: 'json',
                   data: {
                       name: name.val(),
                       ...
                       body: body.val()
                        ...
                        if (response.status == "success")
                            window.location.href = "achievers.php";
                        else {
